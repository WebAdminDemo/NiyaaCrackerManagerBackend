...
const normalizeBrand = (value) => {
  ...
  if (!text) return "";
  if ([BRAND.STANDARD.toLowerCase(), "standard fireworks"].includes(text)) return BRAND.STANDARD;
  if ([BRAND.MULTIBRAND.toLowerCase(), "multi-brand", "multi brand"].includes(text)) return BRAND.MULTIBRAND;
  return "";
};

const getBrandStatus = (value) => {
...
const parseTags = (value) => {
  ...
  return text.split(',').map((brand) => brand.trim()).filter(Boolean);
};
...
function mapProduct(product, index) {
  ...
  const status = cleanString(product.status, PRODUCT_STATUS.IN_STOCK).toLowerCase();
  ...
  return {
    ...
    brand_status: getBrandStatus(product.brand),
    ui_flags: parseJson(
      product.uiFlags,
      {
